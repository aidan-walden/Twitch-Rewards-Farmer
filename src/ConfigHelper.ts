...
export class Config {
  ...
  constructor() {
    ...
    try {
      ...
      if (e.code === 'ENOENT') {
        console.log(
          'No config.json file present! Creating an example one now...'
        );
        const exampleConfig = {
          app: {
            debug: false,
  ...
  constructor() {
    ...
    try {
      const fallbackFile = fs.readFileSync(
        join(__dirname, 'config/fallback.json'),
        'utf8'
      );
      ...
      if (e.code === 'ENOENT') {
        console.log(
          'Fallback is enabled, but fallback.json does not exist. Creating an example one now...'
        );
        ...
        try {
          fs.writeFileSync(
            join(__dirname, 'config/fallback.json'),
            JSON.stringify(exampleFallback, null, 4),
            'utf8'
          );
        } catch (e) {
