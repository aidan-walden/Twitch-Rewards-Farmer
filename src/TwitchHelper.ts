...
module Twitch {
    ...
    export async function getOauthToken (config: Config) {
      const { data } = await axios.post(`https://id.twitch.tv/oauth2/token?client_id=${config.data.twitch.clientid}&client_secret=${config.data.twitch.clientsecret}&grant_type=client_credentials`)
      return data.access_token
    }

    export async function loginToTwitch (browser: Browser, page: Page, config: Config) {
      await auth.setCookies(page, 'twitch')
      await page.goto('https://www.twitch.tv/login', { waitUntil: 'networkidle2' })
      const cookiesStored = fs.existsSync(join(__dirname, 'cookies/cookies-twitch.json'))
      ...
      if (!loggedIn) {
        ...
        let filePath: string
        if (config.data.app.useMultipleAccounts) filePath = join(__dirname, 'cookies/cookies-' + cookiesObj[4].value + '-twitch.json')
        else filePath = join(__dirname, 'cookies/cookies-twitch.json')
    ...
    export async function getStreamer (streamer: string, oauth: string, config: Config): Promise<Stream | null> {
      const headers = {
        'Client-ID': config.data.twitch.clientid,
        Authorization: 'Bearer ' + oauth
      }
      const { data }: { data: TwitchResponse } = await axios.get(`https://api.twitch.tv/helix/streams?user_login=${streamer}`, {
        headers: headers
      })
    ...
    export async function getStreamTags (streamerId: string, oauth: string, config: Config): Promise<Array<Tag>> {
      const headers = {
        'Client-ID': config.data.twitch.clientid,
        Authorization: 'Bearer ' + oauth
      }
      const { data }: { data : TwitchResponse} = await axios.get(`https://api.twitch.tv/helix/streams/tags?broadcaster_id=${streamerId}`, {
        headers: headers
      })
      return data.data as Array<Tag>
    }

    export async function getTopStreamer (oauth: string, config: Config): Promise<Stream | null> {
      const headers = {
        'Client-ID': config.data.twitch.clientid,
        Authorization: 'Bearer ' + oauth
      }
      const { data }: { data: TwitchResponse } = await axios.get(`https://api.twitch.tv/helix/streams?game_id=${config.data.twitch.game}`, {
        headers: headers
      })
    ...
    export async function getFirstStreamerInArray (list: Array<string>, oauth: string, config: Config): Promise<Stream | null> {
    ...
    export async function watchStreamer (browser: Browser, page: Page, streamer: Stream): Promise<void> {
      ...
      const matureButton = await page.$('button[class="player-content-button js-player-mature-accept js-mature-accept-label"]')
      if (matureButton) {
